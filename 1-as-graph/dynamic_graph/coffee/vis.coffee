...
Network = () ->
  ...
  update = () ->
    ...
    # radial layout
    if layout == "radial"
      cats = sortedcats(curNodesData, curLinksData)
      updateCenters(cats)
  ...
  network.updateSearch = (searchTerm) ->
    ...
    node.each (d) ->
      ...
      if searchTerm.length > 0 and match >= 0
        ...
        d.searched = false
        element.style("fill", (d) -> nodeColors(d.cat))
  ...
  setupData = (data) ->
    ...
    circleRadius = d3.scale.sqrt().range([5, 20]).domain(countExtent)
  ...
  # attr is value stored in node, like 'cat'
  ...
  # Returns array of cats sorted based on
  # current sorting method.
  sortedcats = (nodes,links) ->
    cats = []
    if sort == "links"
      counts = {}
      links.forEach (l) ->
        counts[l.source.cat] ?= 0
        counts[l.source.cat] += 1
        counts[l.target.cat] ?= 0
        counts[l.target.cat] += 1
      # add any missing cats that dont have any links
      nodes.forEach (n) ->
        counts[n.cat] ?= 0

      # sort based on counts
      cats = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      # get just names
      cats = cats.map (v) -> v.key
    else
      # sort cats by song count
      counts = nodeCounts(nodes, "cat")
      cats = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      cats = cats.map (v) -> v.key
  ...
  updateCenters = (cats) ->
    if layout == "radial"
      groupCenters = RadialPlacement().center({"x":width/2, "y":height / 2 - 100})
        .radius(300).increment(18).keys(cats)
  ...
  # particular node.
  strokeFor = (d) ->
    d3.rgb(nodeColors(d.cat)).darker().toString()

  # Mouseover tooltip function
  showDetails = (d,i) ->
    content = '<p class="main">' + d.name + '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">' + d.cat + '</span></p>'
